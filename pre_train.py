...
def train_step_masked_prediction(
    ...
    corrupted = {k: v.float() for k, v in corrupted.items()}
...
def validation_step_masked_prediction(
    ...
    return {"loss/rmse": reconstructor_loss.detach()}
...
def main(args, wandb_sweep: bool = False):
    ...
    # Save computed stats for later use in fine-tuning
    stats_key = f"stats_split_mode_{args.split_mode}_ssl"
    stats_path = os.path.join(args.dataset, "stats.pkl")

    if hasattr(args, "ds_info") and "stats" in args.ds_info:
        import pickle
        if os.path.exists(stats_path):
            ...
            existing = {}
        existing[stats_key] = args.ds_info["stats"]
        with open(stats_path, "wb") as f:
            pickle.dump(existing, f)
        print(f"[INFO] Saved stats under key '{stats_key}' to {stats_path}")
